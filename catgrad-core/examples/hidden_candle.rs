...
/// Construct, shapecheck, and interpret the `SimpleMNISTModel` using the Candle backend.
...
fn run_interpreter(
    ...
    println!("Interpreter execution skipped (candle-backend feature not enabled)");
    Ok(())
}
...
impl Def<1, 1> for SimpleMNISTModel {
    ...
    fn path(&self) -> Path {
        path(vec!["model", "hidden_candle"]).expect("invalid model path")
    }
